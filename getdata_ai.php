...
<body>
	<?php
		require_once("connect_ai.php"); // connect to database
		if ($_SERVER['REQUEST_METHOD'] != 'POST'){ //if this is the first instance of executing this page
			...
			$sql= "SELECT count(*) from factors_ai";  // retrieve the description of the driving factor
			$dbRecord = mysqli_query ($conn,$sql);
			$arrRecord = mysqli_fetch_row($dbRecord);
			$numfactors = $arrRecord[0];
			$sql= "SELECT f_desc from factors_ai";  // retrieve the description of the driving factor
            $sql2= "SELECT f_explain from factors_ai";  // retrieve the description of the driving factor
			$dbRecord = mysqli_query ($conn,$sql);
    ...
    <h3> When considering the adoption of AI in higher education in the future...</h3>
    <div class = "non-mobile">    [Hover over underlined phrases for extra clarification] </div>
	<form name="main" action="getdata_ai.php" method="post">
